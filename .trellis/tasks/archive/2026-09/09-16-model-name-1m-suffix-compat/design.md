...
**后果矩阵（探针实测）**：夹具为单分组、单条目，建组后 `mustEnsureInitialPrices` 再读 `GetUpstreamModelDetail` 的计数字段。

| 配置形态 | `reference_count` | `associations.length` | 断言 | 改动前 |
| --- | --- | --- | --- | --- |
| 无别名 | 1 | 1 | 通过 | 通过 |
| 普通别名（无后缀） | 1 | 2 | **失败** | **失败** |
| 单个后缀别名 `[1M]` | 1 | 3 | **失败** | **失败**（当时为 1 vs 2） |
| 上游 ID 自身以 `[1M]` 结尾、无别名 | 1 | 2 | **失败** | 通过（1 vs 1） |

同一探针还验证了前端其余两条计数不变量：`reference_group_count == group_count` 与 `client_model_count == 去重后的 client_model 数` 在**四种形态下全部成立**。冲突被限定在单一字段的口径上。

**结论（比初版更准确）**：这个断言是按「一条记录一个名称」的假设写的——无别名时 1:1 成立。**从 `alias` / `aliases` 字段存在的那一刻起它就不成立了**，与后缀无关：普通别名的 1 vs 2 已经失败。因此本任务**不改变「是否失败」，只改变失败时的名称数**；唯一新跨过边界的是最后一行（上游 ID 自带 `[1M]` 且无别名，原本 1:1 通过）。

这同时说明修复面比预想的小：不必重新定义「引用」的含义，只要让 `reference_count` 与 `associations.length` 口径一致——两者本就该表达同一件事（这个上游模型被多少个客户端可见名称引用），而 `reference_count` 现在数的是配置条目。
