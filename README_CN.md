...
**Claude 适配开关**：分组详情页的模型表格在「对外别名」与「定价」之间有一列「Claude 适配」。打开它等价于为该模型添加别名 `claude-*[1m]`，把所有 Claude 模型名路由到该上游；关闭则移除该别名。该别名本身不在别名框中显示，由开关管理。同一个分组只能有一个模型启用：打开某个模型的开关会自动关掉同组内原本打开的那个。
